...
module TicTacToe
	class Game
		...
		def solicit_move
			"#{current_player.name} '#{current_player.marker}': Enter a number between 1 and 9 to make your move"
		end
		...
		def play
			...
			while true
				...
				puts solicit_move
				x, y = get_move
				board.set_cell(x, y, current_player.marker)
				if board.game_over
...
john = TicTacToe::Player.new({marker: "X", name: "John"})
ashley = TicTacToe::Player.new({marker:"O", name: "Ashley"})
players = [john, ashley]
